...
module.exports = async (req, res) => {
  ...
  try {
    ...
    //
    // Without one this hung for 40+ seconds when fixturedownload.com stopped
    // answering from Vercel's egress, so the function burned its whole execution
    // budget and the browser gave up first. Failing in 8 seconds lets the client
    // move to its fallbacks while the page is still loading.
    ...
    return res.status(504).json({
      error: timedOut ? "upstream timed out" : e.message,
      upstream: "fixturedownload.com",
      hint: timedOut
        ? "The fixture source did not respond. It has been observed refusing datacentre traffic while remaining reachable from browsers."
        : undefined
    });
  }
};
